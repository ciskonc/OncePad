...
- 🔍 **搜索与替换** — VS Code 风格的查找（Ctrl+F）和替换（Ctrl+H），支持正则/大小写敏感/全字匹配；代码和预览模式均可使用，当前匹配高亮区分
...
| 编辑器 | 搜索与替换 | 查找（Ctrl+F）/ 替换（Ctrl+H），支持正则/大小写/全字匹配 |
...
| 快捷键 | 查找 | Ctrl+F / Cmd+F |
| 快捷键 | 替换 | Ctrl+H / Cmd+Alt+F |
| 快捷键 | 查找下一个 / 上一个 | Enter / Shift+Enter |
